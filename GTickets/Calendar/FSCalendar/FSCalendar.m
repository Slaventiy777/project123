...
{   
    ...
    _locale = [[NSLocale alloc] initWithLocaleIdentifier:@"ru_RU"];
    ...
    if (!FSCalendarInAppExtension) {
        ...
        topBorder.backgroundColor = FSCalendarStandardLineColor;
        [self addSubview:topBorder];
        self.topBorder = topBorder;
        
        UIView* bottomBorder = [[UIView alloc] initWithFrame:CGRectZero];
        bottomBorder.backgroundColor = FSCalendarStandardLineColor;
...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        BOOL needsAdjustingBoundingRect = (self.placeholderType != FSCalendarPlaceholderTypeFillSixRows) &&
        ...
        _topBorder.frame = CGRectMake(0, [self preferredHeaderHeight] - 1, self.fs_width, 1);
...
- (CGFloat)preferredHeaderHeight
{
    if (_headerHeight == FSCalendarAutomaticDimension) {
        if (_preferredWeekdayHeight == FSCalendarAutomaticDimension) {
            _preferredHeaderHeight = self.frame.size.width * 50 / 307;
//            if (!self.floatingMode) {
...
//                _preferredWeekdayHeight = self.frame.size.width * 50 / 307;
//            } else {
...
//                _preferredWeekdayHeight = (FSCalendarStandardWeekdayHeight/DIYider)*contentHeight;
                _preferredWeekdayHeight = self.frame.size.width * 42 / 307;
//            } else {
...
- (void)reloadDataForCell:(FSCalendarCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.dateLessToday = date < self.today;
